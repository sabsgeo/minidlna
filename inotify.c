		...
		while( i < length )
		{
			...
			if( event->len )
			{
				...
				if ( event->mask & IN_ISDIR && (event->mask & (IN_CREATE|IN_MOVED_TO)) )
				{
					...
					//if it's a Kwilt mounted path, which contains "0external", we will sleep a few seconds for munting to finish before go ahead with directory scanning
					ptr = strstr(path_buf, "0external");
					if( ptr != 0)
					{
						int j = 0;
						int folder_level = 0;
						...
						if(folder_level == 1) //only apply the sleeping to one level under /home/0external
						{
							DPRINTF(E_DEBUG, L_INOTIFY,  "Sleep a while for USB folder %s to finish mounting.\n", path_buf);
							sleep(5);
						}
					}
