...
insert_file(char *name, const char *path, const char *parentID, int object, media_types types)
{
	...
	if( (types & TYPE_IMAGES) && is_image(name) )
	{
		...
		if(!detailID)
		{
			snprintf(cmdbuf, sizeof(cmdbuf), "/usr/bin/fix-extension \"%s\"", path);
			system(cmdbuf);
		}		
	}
	...
	else if( (types & TYPE_DOCUMENTS) &&  (!is_audio(name) && !is_video(name) && !is_image(name)))
	{
 		orig_name = strdup(name);
		strcpy(base, DOCUMENT_ALL_ID);
		strcpy(class, "item.documentItem");
		detailID = 1;
	}	
...
{
	struct ignore_path_s *ignored_path;
...
ScanDirectory(const char *dir, const char *parent, media_types dir_types)
{
	...
	{
		case ALL_MEDIA:
			n = scandir(dir, &namelist, filter_all, alphasort);
			break;
		case TYPE_AUDIO:
		...
		case TYPE_IMAGES:
			...
			break;
		case TYPE_DOCUMENTS:
			n = scandir(dir, &namelist, filter_d, alphasort);
			break;
		case TYPE_DOCUMENTS|TYPE_IMAGES:
			n = scandir(dir, &namelist, filter_pd, alphasort);
			break;
		case TYPE_DOCUMENTS|TYPE_AUDIO:
			n = scandir(dir, &namelist, filter_ad, alphasort);
			break;
		case TYPE_DOCUMENTS|TYPE_VIDEO:
			n = scandir(dir, &namelist, filter_vd, alphasort);
			break;
		case TYPE_DOCUMENTS|TYPE_IMAGES|TYPE_AUDIO:
			n = scandir(dir, &namelist, filter_apd, alphasort);
			break;	
		case TYPE_DOCUMENTS|TYPE_IMAGES|TYPE_VIDEO:
			n = scandir(dir, &namelist, filter_vpd, alphasort);
			break;
		case TYPE_DOCUMENTS|TYPE_AUDIO|TYPE_VIDEO:
			n = scandir(dir, &namelist, filter_avd, alphasort);
			break;			
		case TYPE_IMAGES|TYPE_AUDIO|TYPE_VIDEO:
...
_notify_start(void)
{
	if (access("/etc/minidlna.d/scan-begin", X_OK) == 0 )
		system("/etc/minidlna.d/scan-begin >/dev/null 2>&1 &");
#ifdef READYNAS
...
_notify_stop(void)
{
	if (access("/etc/minidlna.d/scan-end", X_OK) == 0 )
		system("/etc/minidlna.d/scan-end >/dev/null 2>&1 &");
#ifdef READYNAS
