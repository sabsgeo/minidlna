...
is_video(const char * file)
{
	return (ends_with(file, ".mpg") || ends_with(file, ".mpeg")  ||
		...
		ends_with(file, ".3gpp")|| ends_with(file, ".3gpp2") ||
		ends_with(file, ".3gp2")|| ends_with(file, ".dv")    ||
		ends_with(file, ".ogv") || ends_with(file, ".oggtheora") ||
		ends_with(file, ".rm")  || ends_with(file, ".webm"));
}
			...
			switch( dir_type )
			{
				...
				case TYPE_IMAGES:
					if( is_image(path) )
						type = TYPE_FILE;
					break;
				case TYPE_IMAGES|TYPE_VIDEO:
					if( is_image(path) || is_video(path) )
						type = TYPE_FILE;
					break;
				case TYPE_IMAGES|TYPE_AUDIO:
					if( is_image(path) || is_audio(path) )
						type = TYPE_FILE;
					break;
				case TYPE_VIDEO|TYPE_AUDIO:
					if( is_video(path) || is_audio(path) )
						type = TYPE_FILE;
					break;
				default:
					break;
